// Function to retrieve a part of a string a specified indexes: the beginning (inclusive).
...
String_substring2:
	...
	MOV X2, X0       // Copies the string's address into X2
	BL String_length // Calls function to get the string's length. Value returned at X0

        SUB X19, X0, X1  // The length of the substring = String's length - beginIndex
	...
	MOV X0, X2           // Moves the string's address into X0
	BL String_length     // Calls function to get the string's length. Value returned at X0

        SUB X20, X0, X1      // Substring's length = String's length - begIndex (sentinel value)
...
loop:
        LDRB W22, [X2, X1]   // Loads byte from string starting at BEGINNING index into X22
