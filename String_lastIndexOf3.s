// This function will find the first occurence of a string found within a string.
...
secondSearch:
	...
	LDRB W20, [X20, X24] // Loads the next byte from checkString
	LDRB W23, [X19, X22] // Loads char byte from string into X23

	ADD X24, X24, #1	// Increment second counter

	CMP X20, X23	// Compare the lettes
	B.NE reenter	// jump back into the search loop

	CMP X21, #0          // Compares the string's length with 0
	B.EQ notFound        // If string has reached 0, then we skipped to the not found section

	CMP x24, #4
	B.EQ end	// Restart secondSearch
